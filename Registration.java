...
class MyFrame
	...
	public MyFrame() throws IOException {
		this.op = new Operations("detailsRam.txt");
		...
		bran.setFont(new Font("Arial", Font.PLAIN, 20));
		bran.setSize(100, 20);
		bran.setLocation(100, 300);
		c.add(bran);

		branch = new JComboBox(branches);
		branch.setFont(new Font("Arial", Font.PLAIN, 15));
		branch.setSize(60, 20);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == sub) {
			if (term.isSelected()) {
				...
				String brnch = "" + branch.getSelectedItem();

				try {
					// FileWriter fWriter = new FileWriter(
					// "details.txt", true);
					// fWriter.write(data + data1 + data2 + data3 + data4 + "\n");
					String dataToWrite[] = { name, pNo, gndr, dob, addr, brnch };
					int res = this.op.createRecord(dataToWrite);
					// fWriter.close();
